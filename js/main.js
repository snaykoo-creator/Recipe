(function () {
  ...
  var DEFAULT_AVATAR_SRC = "images/prof.jpg";

  var STORAGE_USER = "recipe_demo_user";
  var STORAGE_USERS = "recipe_demo_users";
  var STORAGE_SITE_SETTINGS = "recipe_site_settings";
  ...
  function getSiteSettings() {
    try {
      var raw = localStorage.getItem(STORAGE_SITE_SETTINGS);
      if (!raw) return { bannerOn: false, bannerText: "", bannerId: 0 };
      var o = JSON.parse(raw);
      return {
        bannerOn: !!o.bannerOn,
        bannerText: typeof o.bannerText === "string" ? o.bannerText : "",
        bannerId: typeof o.bannerId === "number" ? o.bannerId : 0
      };
    } catch (e) {
      return { bannerOn: false, bannerText: "", bannerId: 0 };
    }
  }

  function setSiteSettings(obj) {
    var prev = getSiteSettings();
    var merged = {
      bannerOn: obj.bannerOn !== undefined ? !!obj.bannerOn : prev.bannerOn,
      bannerText: obj.bannerText !== undefined ? String(obj.bannerText || "") : prev.bannerText,
      bannerId: obj.bannerId !== undefined ? obj.bannerId : prev.bannerId
    };
    localStorage.setItem(STORAGE_SITE_SETTINGS, JSON.stringify(merged));
  }

  /* ---------- Объявление с админки (все страницы с шапкой) ---------- */
  function initSiteBanner() {
    if (document.body.classList.contains("page--admin")) return;

    var settings = getSiteSettings();
    if (!settings.bannerOn || !settings.bannerText.trim()) return;

    var dismissKey = "recipe_banner_dismiss_" + String(settings.bannerId || 0);
    ...
    bar.setAttribute("aria-label", "Объявление");
    bar.innerHTML =
      '<div class="site-banner__inner container">' +
      '<p class="site-banner__text"></p>' +
      '<button type="button" class="site-banner__close" aria-label="Скрыть до обновления страницы">×</button>' +
      "</div>";

    var textEl = bar.querySelector(".site-banner__text");
    if (textEl) textEl.textContent = settings.bannerText;

    var closeBtn = bar.querySelector(".site-banner__close");
    if (closeBtn) {
